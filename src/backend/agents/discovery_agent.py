...
class DiscoveryAgent:
    def __init__(self, github_token=None):
        ...
        if not self.token:
            rprint("[bold magenta][DiscoveryAgent][/bold magenta] [bold red]ERROR:[/bold red] GITHUB_TOKEN not found.")
    ...
    def search_high_priority_bugs(self):
        ...
        rprint("[bold magenta][DiscoveryAgent][/bold magenta] Searching GitHub for issues (small and simplified projects)...")
        ...
        if os.path.exists(self.cache_file):
            with open(self.cache_file, "r", encoding='utf-8') as f:
                ...
                if time.time() - cache.get("timestamp", 0) < 3600:
                    rprint("[bold magenta][DiscoveryAgent][/bold magenta] Utilizing cached results.")
        ...
        with console.status("[bold magenta][DiscoveryAgent][/bold magenta] Searching GitHub for issues..."):
        ...
        if issues.totalCount == 0:
            rprint("[bold magenta][DiscoveryAgent][/bold magenta] No issues found.")
            return None
        ...
        if not issue_obj:
            rprint("[bold magenta][DiscoveryAgent][/bold magenta] All found issues have already been processed.")
            return None
        issue_data = {
        ...
        rprint(f"[bold magenta][DiscoveryAgent][/bold magenta] Found Issue #{issue_obj.number} in {issue_obj.repository.full_name}")
        return issue_data
    ...
    def search_specific_repo_bugs(self, target_url):
        ...
        rprint(f"[bold magenta][DiscoveryAgent][/bold magenta] Searching issues for target: {target_url} ...")
        ...
        rprint(f"[bold magenta][DiscoveryAgent][/bold magenta] DEBUG: repo_name={repo_name}, issue_number={issue_number}")
        try:
            ...
            if issue_number:
                ...
                if issues.totalCount > 0:
                    ...
                    if issues.totalCount > 0:
                        ...
                        rprint("[bold magenta][DiscoveryAgent][/bold magenta] No issues found in the specified repo.")
                        return None
            ...
            rprint(f"[bold magenta][DiscoveryAgent][/bold magenta] Found Issue #{issue_obj.number} in {repo.full_name}")
            return issue_data
            
        except Exception as e:
            rprint(f"[bold magenta][DiscoveryAgent][/bold magenta] ERROR while retrieving repo/issue: {e}")
            return None
