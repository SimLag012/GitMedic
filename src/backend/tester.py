...
def run_tests(modified_file_path, python_exe="python"):
    ...
    rprint(f"[bold red][Tester][/bold red] Syntax check: {modified_file_path}")
    ...
    if syntax_result.returncode != 0:
        ...
        rprint(f"Syntax check failed:\n{error_msg}")
        return False, error_msg

    rprint("Syntax check (py_compile) passed.")
    ...
    rprint(f"Attempting targeted pytest in {target_dir}...")
    result = subprocess.run([python_exe, "-m", "pytest", target_dir], capture_output=True, text=True)
    
    if result.returncode == 0:
        rprint("Targeted tests completed successfully.")
        ...
        if result.returncode == 5:
            rprint("No tests found in target directory. Proceeding with syntax only.")
            return True, "Syntax OK (No tests found)"
        
        rprint(f"Targeted pytest failed (code {result.returncode}).")
